...
int main() {
  ...
  for(i = 0; i < n;i++)
  {
  	if(arr[i] > 0 && arr[i] % 2 !=0) //Mang so nguyen b[] ghi lai so lan xuat hien cua
  		b[arr[i]]++;			//cac phan tu lon hon 0 trong day arr
  	if(arr[i] < 0 && arr[i] % 2 !=0) //Mang so nguyen c[] ghi lai so lan xuat hien cua
  ...
  for(i = 0; i <n ;i++)
  {
  	...
  	{
  		if(b[arr[i]] > max && arr[i] % 2 !=0 )
  			max = b[arr[i]];
  	}
  	else
  	{
  		if(c[-arr[i]] > max && arr[i] % 2 !=0)
  			max = c[-arr[i]];
  	}
